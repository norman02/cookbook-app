...
jest.mock("mongodb", () => {
  ...
  const mockClient = { db: jest.fn(() => mockDb), connect: jest.fn(), close: jest.fn() };
...
describe("DBStorage Implementation", () => {
  ...
  test("should save recipes to the database", async () => {
    const mockRecipes = [{ name: "Pasta", ingredients: ["noodles", "sauce"], instructions: "Boil and mix." }];
    const result = await DBStorage.saveRecipes(mockRecipes);
    expect(result).toBe(true);
  });

  test("should handle database connection errors gracefully", async () => {
    jest.spyOn(DBStorage, "getRecipes").mockRejectedValue(new Error("Database connection failed"));
    await expect(DBStorage.getRecipes()).rejects.toThrow("Database connection failed");
  });
});
